...
func parseWASubFormattingToHTML(text string, allowInlineURL bool, output *strings.Builder) {
	...
	for i, line := range lines {
		...
		if strings.HasPrefix(line, "* ") || strings.HasPrefix(line, "- ") {
			...
			_, _ = fmt.Fprintf(
				output, "<li>%s</li>", parseWASubFormattingLineToHTML(line[2:], allowInlineURL),
			)
			...
			parsedLine := parseWASubFormattingLineToHTML(
				strings.TrimPrefix(line, match[0]), allowInlineURL,
			)
			...
			_, _ = fmt.Fprintf(
				...
				parseWASubFormattingLineToHTML(line[2:], allowInlineURL),
			)
			wasBlockQuote = true
		} else {
			if orderedListIdx >= 0 {
