...
func init() {
	if strings.ReplaceAll(
		...
		getPollOptionQueryArrayPlaceholder,
		"meow",
	) == getPollOptionIDsByHashesQuery {
		panic("Array select query placeholder not found")
	}
	if strings.ReplaceAll(
		getPollOptionHashesByIDsQuery,
		getPollOptionQueryArrayPlaceholder,
		"meow",
	) == getPollOptionIDsByHashesQuery {
		panic("Array select query placeholder not found")
...
func (poq *PollOptionQuery) Put(
	ctx context.Context,
	mxid id.EventID,
	opts map[[32]byte]string,
) error {
	if len(opts) == 0 {
		return nil
	}
...
func (poq *PollOptionQuery) GetIDs(
	ctx context.Context,
	mxid id.EventID,
	hashes [][]byte,
) (map[[32]byte]string, error) {
...
func (poq *PollOptionQuery) GetHashes(
	ctx context.Context,
	mxid id.EventID,
	ids []string,
...
func getPollOptions[LookupKey any, Key comparable, Value any](
	...
	if poq.Dialect == dbutil.Postgres {
		...
		query = strings.ReplaceAll(
			...
			getPollOptionQueryArrayPlaceholder,
			fmt.Sprintf(
				getPollOptionQuerySQLiteArrayTemplate,
				strings.TrimSuffix(strings.Repeat("?,", len(things)), ","),
			),
		)
